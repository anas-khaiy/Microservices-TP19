...
public class VoitureController {
    ...
    @Autowired
    ClientService clientService; // Fixed: Uses the interface, not the Application class
    ...
    public ResponseEntity<Object> findAll() {
        try {
            ...
            for (Voiture v : voitures) {
                // Fetch the client data from the microservice
                if (v.getClientId() != null) {
                    try {
                        v.setClient(clientService.clientById(v.getClientId()));
                    } catch (Exception e) {
                        // Log error or ignore if client service is down
                    }
                }
            }
    ...
    public ResponseEntity<List<Voiture>> findByClient(@PathVariable Long id) {
        try {
            ...
            if (client != null) {
                // Ensure Repository has findByClientId(Long id)
    ...
    public ResponseEntity<Object> save(@PathVariable Long clientId, @RequestBody Voiture voiture) {
        try {
            ...
            if (client != null) {
                ...
                voiture.setClientId(clientId); // Set the FK for database
    ...
    public ResponseEntity<Object> update(@PathVariable Long id, @RequestBody Voiture updatedVoiture) {
        try {
            ...
            // Note: clientId is usually not updated here, but can be added if needed
